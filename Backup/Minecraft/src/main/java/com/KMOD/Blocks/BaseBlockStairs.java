...
public class BaseBlockStairs extends BaseBlock
{
    public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
    public static final PropertyEnum HALF = PropertyEnum.create("half", BaseBlockStairs.EnumHalf.class);
    public static final PropertyEnum SHAPE = PropertyEnum.create("shape", BaseBlockStairs.EnumShape.class);
    ...
    public BaseBlockStairs(String Name, IBlockState modelState, float hardness, float resistance)
    {
        super(Name, hardness, resistance);
        this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(HALF, BaseBlockStairs.EnumHalf.BOTTOM).withProperty(SHAPE, BaseBlockStairs.EnumShape.STRAIGHT));
        this.modelBlock = modelState.getBlock();
        this.modelState = modelState;
        this.setStepSound(this.modelBlock.stepSound);
        this.setLightOpacity(255);
        this.setCreativeTab(KCore.OHTab);
        this.setMaterial(modelState.getBlock().getMaterial());
    }
    ...
    {
        if (worldIn.getBlockState(pos).getValue(HALF) == BaseBlockStairs.EnumHalf.TOP)
        {
    ...
    {
        return p_150148_0_ instanceof BaseBlockStairs;
    }
    ...
    public int func_176307_f(IBlockAccess p_176307_1_, BlockPos p_176307_2_)
    {
        ...
        BaseBlockStairs.EnumHalf enumhalf = (BaseBlockStairs.EnumHalf)iblockstate.getValue(HALF);
        boolean flag = enumhalf == BaseBlockStairs.EnumHalf.TOP;
    ...
    public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer)
    {
        ...
        iblockstate = iblockstate.withProperty(FACING, placer.getHorizontalFacing()).withProperty(SHAPE, BaseBlockStairs.EnumShape.STRAIGHT);
        return facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double)hitY <= 0.5D) ? iblockstate.withProperty(HALF, BaseBlockStairs.EnumHalf.BOTTOM) : iblockstate.withProperty(HALF, BaseBlockStairs.EnumHalf.TOP);
    }
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end)
    {
        ...
        boolean flag = iblockstate.getValue(HALF) == BaseBlockStairs.EnumHalf.TOP;
    ...
    {
        IBlockState iblockstate = this.getDefaultState().withProperty(HALF, (meta & 4) > 0 ? BaseBlockStairs.EnumHalf.TOP : BaseBlockStairs.EnumHalf.BOTTOM);
    ...
    public int getMetaFromState(IBlockState state)
    {
        ...
        if (state.getValue(HALF) == BaseBlockStairs.EnumHalf.TOP)
        {
            i |= 4;
        }
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
    {
        if (this.func_176306_h(worldIn, pos))
        {
            ...
            {
                case 0:
                    state = state.withProperty(SHAPE, BaseBlockStairs.EnumShape.STRAIGHT);
                    break;
                case 1:
                    state = state.withProperty(SHAPE, BaseBlockStairs.EnumShape.INNER_RIGHT);
                    break;
                case 2:
                    state = state.withProperty(SHAPE, BaseBlockStairs.EnumShape.INNER_LEFT);
            }
        }
        else
        {
            switch (this.func_176307_f(worldIn, pos))
            {
                case 0:
                    ...
                    break;
                case 1:
                    state = state.withProperty(SHAPE, BaseBlockStairs.EnumShape.OUTER_RIGHT);
                    break;
                case 2:
                    state = state.withProperty(SHAPE, BaseBlockStairs.EnumShape.OUTER_LEFT);
            }
        }
