...
public class TeleporterHeaven extends Teleporter
{
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? Blocks.end_stone.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
    {
            ...
            for (int l = -128; l <= 128; ++l)
            {
                ...
                for (int i1 = -128; i1 <= 128; ++i1)
                {
                    for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
                    {
                        ...
                        if (this.worldServerInstance.getBlockState(blockpos).getBlock() == KCore.HeavenlyPortal)
                        {
                            while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == KCore.HeavenlyPortal)
                            {
                                blockpos = blockpos1;
                            }
        ...
        if (d0 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlockState(((BlockPos)object).west()).getBlock() == KCore.HeavenlyPortal)
            {
                enumfacing = EnumFacing.NORTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).east()).getBlock() == KCore.HeavenlyPortal)
            {
                enumfacing = EnumFacing.SOUTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).north()).getBlock() == KCore.HeavenlyPortal)
            {
                enumfacing = EnumFacing.EAST;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).south()).getBlock() == KCore.HeavenlyPortal)
            {
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        boolean flag = k3 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? Blocks.end_stone.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = KCore.HeavenlyPortal.getDefaultState();
        ...
        for (j3 = 0; j3 < 4; ++j3)
        {
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.end_stone.getDefaultState() : iblockstate, 2);
                }
            }
